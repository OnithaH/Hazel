import serial
import subprocess
import time
import os
import json

# Try to import smbus2 for UPS HAT B
try:
    import smbus2
except ImportError:
    print("⚠️ smbus2 not found. Run: pip install smbus2")

# --- 1. CONFIGURATION & PATHS ---
BASE_PATH   = "/home/hazel123/Documents/hazel_rasp"
ENV_FACE    = f"{BASE_PATH}/hazel_face/venv/bin/python3"
ENV_GENERAL = f"{BASE_PATH}/general_mode/venv/bin/python" 
ENV_STUDY   = f"{BASE_PATH}/study_mode/venv/bin/python"
ENV_GAME    = f"{BASE_PATH}/game_mode/venv/bin/python"
ENV_MUSIC   = f"{BASE_PATH}/music_mode/venv/bin/python"

# Communication Files
CMD_FILE    = "/tmp/hazel_web_cmd.txt"       # Mailbox for Web -> Robot
STATUS_FILE = "/tmp/hazel_sensor_data.json"  # Mailbox for Robot -> Web
CONVO_NAME  = "hazel-live-convo"

# Hardware Settings
UPS_ADDR = 0x42 
bus = None
ser = None
active_process = None
face_process   = None 

# --- 2. HARDWARE HELPERS ---
try:
    bus = smbus2.SMBus(1)
except Exception as e:
    print(f"❌ I2C Error: {e}")

def get_voltage():
    if not bus: return 0.0
    try:
        read = bus.read_word_data(UPS_ADDR, 0x02)
        swapped = ((read << 8) & 0xFF00) | ((read >> 8) & 0x00FF)
        voltage = (swapped >> 3) * 0.004 
        return round(voltage, 2)
    except: return 0.0

def save_telemetry(v):
    """Saves current state so the DB Sync Worker can upload it to Aiven."""
    data = {
        "voltage": v,
        "timestamp": time.time(),
        "status": "online"
    }
    with open(STATUS_FILE, "w") as f:
        json.dump(data, f)

# --- 3. PROCESS MANAGEMENT ---
def run_program(script_path, venv_path, needs_face=True, needs_convo=False):
    global active_process, face_process, ser
    
    # A. Stop existing mode
    if active_process:
        active_process.terminate()
        try: active_process.wait(timeout=1.0)
        except: active_process.kill()
        active_process = None

    # B. Face Persistence (Keep ON for General/Study, OFF for others)
    if needs_face:
        if not face_process:
            print("😊 Opening Hazel Face...")
            face_script = f"{BASE_PATH}/hazel_face/face.py"
            face_process = subprocess.Popen([ENV_FACE, face_script])
    else:
        if face_process:
            print("😴 Closing Hazel Face...")
            face_process.terminate()
            face_process = None

    # C. Handle Live Conversation (Managed via PM2)
    if needs_convo:
        os.system(f"pm2 start {CONVO_NAME}")
    else:
        os.system(f"pm2 stop {CONVO_NAME}")

    if ser: ser.reset_input_buffer()
    
    # D. Launch Mode
    full_path = f"{BASE_PATH}/{script_path}"
    print(f"🚀 Launching Mode: {script_path}")
    active_process = subprocess.Popen([venv_path, full_path])

def check_web_mailbox():
    """Checks if the db_sync_worker dropped a web command."""
    if os.path.exists(CMD_FILE):
        try:
            with open(CMD_FILE, "r") as f:
                cmd = f.read().strip()
            os.remove(CMD_FILE) # Consume the message
            return cmd
        except: return None
    return None

# --- 4. INITIALIZATION ---
try:
    ser = serial.Serial('/dev/ttyAMA0', 115200, timeout=1)
    time.sleep(2) 
    ser.write(b'1') # Trigger JBL Power ON
    print("✅ Hardware Ready")
except Exception as e:
    print(f"❌ Serial Error: {e}")

# Startup: General Mode (Face ON, Convo ON)
run_program("general_mode/general_mode.py", ENV_GENERAL, needs_face=True, needs_convo=True)

# --- 5. MAIN LOOP ---
try:
    last_sync = 0
    while True:
        # A. Telemetry & Battery Safety (Every 10s)
        current_v = get_voltage()
        if time.time() - last_sync > 10:
            print(f"🔋 Battery: {current_v}V")
            save_telemetry(current_v)
            last_sync = time.time()

        if 0.1 < current_v < 6.4: # Shutdown for 2S Li-ion
            ser.write(b'0') # JBL OFF
            os.system("sudo shutdown now")

        # B. Check Physical Inputs (ESP32 UART)
        if ser and ser.in_waiting > 0:
            raw = ser.read(ser.in_waiting).decode('utf-8', errors='ignore')
            if "MODE_GENERAL" in raw: run_program("general_mode/general_mode.py", ENV_GENERAL, True, True)
            elif "MODE_STUDY" in raw: run_program("study_mode/study_mode.py", ENV_STUDY, True, False)
            elif "MODE_GAME" in raw:  run_program("game_mode/game_mode.py", ENV_GAME, False, False)
            elif "MODE_MUSIC" in raw: run_program("music_mode/music_mode.py", ENV_MUSIC, False, False)
            elif "Vol up" in raw:     os.system("amixer set Master 5%+")
            elif "Vol down" in raw:   os.system("amixer set Master 5%-")

        # C. Check Web Inputs (Internal Mailbox)
        web_cmd = check_web_mailbox()
        if web_cmd:
            print(f"🌐 Processing Web Command: {web_cmd}")
            # Example triggers: AROMA_LAVENDER, START_STUDY, etc.
            if "AROMA" in web_cmd:
                ser.write(b'A') # Send Aroma signal to ESP32
            elif "START_STUDY" in web_cmd:
                run_program("study_mode/study_mode.py", ENV_STUDY, True, False)

        time.sleep(0.01)

except KeyboardInterrupt:
    if active_process: active_process.kill()
    if face_process: face_process.kill()